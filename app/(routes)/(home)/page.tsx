...
export default async function Home() {
  ...
  let featuredChallenges = await Challenge.find({ isFeatured: true }).limit(3).lean() as ChallengeItem[];
  ...
  const completedChallengesAggregation = await User.aggregate([
    { $unwind: "$completedChallenges" },
    { $match: { "completedChallenges.status": "Başarılı" } },
    { $count: "total" }
  ]);

  const totalCompletedChallenges = completedChallengesAggregation.length > 0 
    ? completedChallengesAggregation[0].total 
    : 0;
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            <div>
              ...
              <span className="text-4xl font-bold text-brand-primary">
                {totalChallenges.toLocaleString('tr-TR')}
              </span>
            </div>
            ...
            <div>
              ...
              <span className="text-4xl font-bold text-brand-primary">
                {totalCompletedChallenges.toLocaleString('tr-TR')}
              </span>
            </div>
          </div>
        </div>
